...
if ($Help) {
    Write-Output "Usage: bundle-windows-ci.ps1 [-Install] [-Help]"
    Write-Output "Build the installer for Windows in CI environment.\n"
...
function GenerateLicenses {
    ...
    if (Test-Path "$PSScriptRoot/generate-licenses.ps1") {
        . $PSScriptRoot/generate-licenses.ps1
    } else {
        Write-Output "Skipping license generation in CI"
    }
...
function CreateDummySignScript {
    $signScript = @"
# Dummy sign script for CI build (no actual signing)
param([string]`$files)
Write-Host "Skipping code signing for files: `$files"
exit 0
"@
    Set-Content -Path "$innoDir\sign.ps1" -Value $signScript -Force
}
...
function MakeAppx {
    ...
    if (Test-Path $manifestFile) {
        ...
        if (Test-Path $sdk) {
            ...
            Write-Output "Windows SDK not found, skipping AppX creation"
            # Create dummy appx file
            New-Item -Path "$innoDir\zed_explorer_command_injector.appx" -ItemType File -Force
        }
    } else {
        Write-Output "AppX manifest not found, skipping AppX creation"
        # Create dummy appx file
        New-Item -Path "$innoDir\zed_explorer_command_injector.appx" -ItemType File -Force
    }
}
...
function DownloadAMDGpuServices {
    try {
        ...
        Write-Warning "Failed to download AMD GPU Services SDK: $_"
        # Create dummy directory structure
        New-Item -Path ".\AGS_SDK-6.3.0\ags_lib\lib" -ItemType Directory -Force
        # Create dummy dll file
        New-Item -Path ".\AGS_SDK-6.3.0\ags_lib\lib\amd_ags_x64.dll" -ItemType File -Force
    }
}
